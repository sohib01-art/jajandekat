...
async function ensurePushSubscription(opts = {}) {
  ...
  try {
    ...
    if (!sub) {
      // Izin notifikasi browser sudah 'granted', tapi langganan push-nya sendiri hilang/kedaluwarsa
      // (biasa terjadi setelah Service Worker diperbarui, cache/data situs dibersihkan sebagian, dst).
      // Ini penyebab paling umum notifikasi "mati sendiri": kalau baris ini di-skip begitu saja saat
      // silent=true, device itu TIDAK PERNAH langganan ulang secara otomatis — makanya harus tetap
      // dicoba di sini. requestPermission() aman dipanggil lagi di sini: kalau izin sudah 'granted'
      // (termasuk saat silent), browser langsung mengembalikan 'granted' tanpa memunculkan dialog apa pun.
...
window.__notifTap = async function () {
  ...
  if (Notification.permission === 'granted') {
    // Perbaikan sendiri: izin browser sudah aktif, tapi langganan push bisa saja sudah kedaluwarsa
    // tanpa sepengetahuan pengguna (lihat catatan di ensurePushSubscription). Ketukan lonceng jadi
    // kesempatan untuk diam-diam membuat ulang langganannya kalau memang sudah hilang.
...
window.__openRegister = function () {
  regOpen = true;
  ...
  if (!myVendorId) {
    ...
    main.innerHTML = `
      ${vendors.length ? `
        <div class="lg-card">
          <div class="lg-top">
            <div class="lg-txt"><h2>Sudah punya<br>akun <span>pedagang?</span></h2><p>Masuk untuk mengaktifkan jualanmu kembali.</p></div>
            <div class="lg-mascot"><img src="icons/maskot.png" alt="" /></div>
          </div>
          <label class="lg-label" for="pick-whatsapp"><span class="lg-ic"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="12" fill="#25D366"/><path fill="#fff" d="M12 5.2a6.8 6.8 0 0 0-5.85 10.27L5.2 18.8l3.42-.9A6.8 6.8 0 1 0 12 5.2Zm3.9 9.5c-.17.48-.98.92-1.35.95-.35.03-.68.16-2.3-.48-1.95-.77-3.2-2.75-3.3-2.88-.1-.13-.79-1.05-.79-2s.5-1.42.68-1.61c.17-.19.38-.24.5-.24h.36c.12 0 .27-.04.42.32.16.38.53 1.32.58 1.42.05.1.08.21.02.34-.07.13-.1.21-.2.33l-.3.35c-.1.1-.2.2-.09.4.12.19.52.86 1.12 1.39.77.69 1.42.9 1.62 1 .2.1.32.08.43-.05.12-.14.5-.58.63-.78.13-.2.27-.16.45-.1.18.07 1.16.55 1.36.65.2.1.33.15.38.24.05.08.05.5-.12.98Z"/></svg></span>Nomor WhatsApp</label>
          <div class="lg-field"><svg class="lg-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 16.9v3a2 2 0 0 1-2.2 2 19.8 19.8 0 0 1-8.6-3.1 19.5 19.5 0 0 1-6-6A19.8 19.8 0 0 1 2.1 4.2 2 2 0 0 1 4.1 2h3a2 2 0 0 1 2 1.7c.1 1 .4 1.9.7 2.8a2 2 0 0 1-.5 2.1L8.1 9.9a16 16 0 0 0 6 6l1.3-1.3a2 2 0 0 1 2.1-.4c.9.3 1.8.6 2.8.7a2 2 0 0 1 1.7 2Z"/></svg><input id="pick-whatsapp" type="tel" value="${pickWhatsappValue.replace(/"/g, '&quot;')}" oninput="window.__updatePickWhatsapp(this.value)" placeholder="08xxxxxxxxxx" autocomplete="tel" enterkeyhint="next" /></div>
          <div class="lg-hint">Nomor akan otomatis disesuaikan ke +62</div>
          <label class="lg-label" for="pick-pin"><span class="lg-ic"><svg viewBox="0 0 24 24" aria-hidden="true"><rect width="24" height="24" rx="7" fill="#FF7A2F"/><path fill="#fff" d="M12 5.5a3 3 0 0 0-3 3V10H8.2c-.66 0-1.2.54-1.2 1.2v5.6c0 .66.54 1.2 1.2 1.2h7.6c.66 0 1.2-.54 1.2-1.2v-5.6c0-.66-.54-1.2-1.2-1.2H15V8.5a3 3 0 0 0-3-3Zm-1.5 3a1.5 1.5 0 0 1 3 0V10h-3V8.5Z"/></svg></span>PIN</label>
          <div class="lg-field"><svg class="lg-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg><input id="pick-pin" type="password" inputmode="numeric" maxlength="6" placeholder="Masukkan PIN 6 digit" autocomplete="current-password" enterkeyhint="go" /><button type="button" class="lg-eye" aria-label="Tampilkan PIN" onclick="var i=document.getElementById('pick-pin'); i.type = i.type === 'password' ? 'tel' : 'password'"><svg class="lg-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2 12s3.6-7 10-7 10 7 10 7-3.6 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg></button></div>
          <div id="pick-error" class="lg-error"></div>
          <button type="button" class="lg-submit" onclick="window.__pickVendor()"><svg class="lg-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4M10 17l5-5-5-5M15 12H3"/></svg><span>MASUK KE AKUN</span></button>
          <a href="#" class="lg-forgot" onclick="window.__forgotPin(); return false;">Lupa PIN?</a>
          <div class="lg-or"><span>atau</span></div>
          <button type="button" class="lg-reg" id="lg-reg-btn" ${regVisible ? 'hidden' : ''} onclick="window.__openRegister()"><svg class="lg-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3.5 9.5 5 4h14l1.5 5.5M3.5 9.5a2.8 2.8 0 0 0 5.5 0 2.8 2.8 0 0 0 6 0 2.8 2.8 0 0 0 5.5 0M5 12.5V20h14v-7.5"/></svg><span>Daftar sebagai pedagang baru</span><b>›</b></button>
        </div>
        <img class="lg-banner" src="icons/banner-pedagang.png" alt="JajanDekat: Jajanan Favorit, Peluang Besar untuk Pedagang Lokal" />
      ` : ''}

      <div class="vendor-hero" id="reg-section" ${regVisible ? '' : 'hidden'}>
...
async function init() {
  ...
  try {
    ...
    // Ketuk notifikasi saat tab sudah terbuka -> Service Worker minta tab ini navigasi ke tujuannya
    // (lihat notificationclick di sw.js); tanpa listener ini, tab cuma ke-fokus tanpa berpindah halaman.
